...
| 4 | ~~Native internal representation: carry FP values internally in a format matching the CPU's 80-bit extended register (explicit integer bit, 16-bit exponent) instead of packed IEEE double, so `fmove` to/from an FPn register needs no hidden-bit insert/strip; convert to IEEE double/single only when writing to memory in that format~~ | 1 | ❌ Not as scoped | `claude/keen-mendel-3hb6vs` | Design + measurement pass, no op files touched (see `DESIGN-04-native-extended-repr.md`, and `bench/`'s new `fmove`/`fmovem` memory-operand probe with real numbers). Register-to-register `fmove` is already a straight copy today — the checklist item's own premise doesn't hold there. Measured that the actual `.x`-format cost is real but **relocates rather than disappears**: `fmove.d` is already cheaper than `fmove.x` today (605-695 vs 732-844 cycles) precisely because the internal format is double, so swapping to extended flips which one is cheap — a wash for a single `fmove`, not a win, unless real workloads are demonstrably `.x`-heavy (open question, not resolvable by more local measurement). The one place with a clear, non-wash win is `fmovem`, which pays the conversion once per register in the list (measured 1369-1457 cycles for just 4 registers). Recommend a narrower follow-up targeting `fmovem` specifically instead of the full representation swap — see the design doc's "Recommendation". Also found, not fixed: a real pre-existing rounding bug in `ExtendedToDouble` (logged in `ISSUES.md`), independent of this row. |
